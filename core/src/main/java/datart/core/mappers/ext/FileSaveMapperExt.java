...
public interface FileSaveMapperExt extends FileSaveMapper {
    ...
    @Select({
            ...
            "SELECT\n" +
                    ...
                    "</foreach>\n" +
                    "</if>\n" +
                    "<if test=\"isFolder != null \"> AND filesave.is_folder = #{isFolder}</if>\n" +
                    "<if test=\"parentId != null \"> AND filesave.parent_id = #{parentId}</if>\n" +
                    "<if test=\"fileName != null  and fileName != ''\">\n" +
                    "AND filesave.file_name like concat('%', #{fileName}, '%')\n" +
                    "</if>",
            "</script>"
    })
